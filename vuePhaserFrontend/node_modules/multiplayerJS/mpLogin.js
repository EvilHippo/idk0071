...
newPlayer.onclick = function () {
    ...
    game.state.start('mpMakeAccount');
};
...
function userLogIn()
{
    playerName = logInUsername.value;
    playerPassword = logInPassword.value;

    $.ajax({
        type: "POST",
        url: "http://localhost:8080/login",
        //data: "{'username':" + playerName + ",'password':" + playerPassword + "}",
        data: "{'username':" + playerName + ",'password':" + playerPassword + "}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function(data)
        {
            console.log('sisselogimise success: ' + data.success);
            if(data.success == false)
            {
                this.logInError = game.add.text(100, 100, 'You entered wrong username or password.', {font: "15px Arial", fill: "#950010"});
            }
            else if(data.success == true)
            {
                logInDiv.style.display = 'none';
                userAccount.style.display = 'block';
                game.state.start('userProfile');
...
function waitGame() {
    ...
    $.ajax({
        ...
        data: "{'UID':" + playerId + ", 'username': " + playerName +  ", 'registeredUser': {'username': " + playerName + "}}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
    });
...
preload: function()
    {
        ...
        game.load.video('bg9', '../farshockAssets/loginBackground.mp4');
        game.load.audio('music', '../farshockAssets/music.m4a');
    },
    create: function () {
        ...
        this.farshockLabel = game.add.text(300,50, 'FARSHOCK', {font: "30px Arial", fill:"#d5efd9"});
        this.introLabel = game.add.text(300,85, 'Your country needs YOU!\n         Enlist today!', {font: "15px Arial", fill:"#d5efd9"});
