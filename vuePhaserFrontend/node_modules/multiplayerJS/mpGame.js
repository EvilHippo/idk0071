...
let newPlayerUsername;
let newPlayerPassword;
...
function movePlayers(serverJson) {
    if(serverJson.id == enemyID) {
        if((Math.abs(players[enemyID].x - serverJson.idCordsX) > 30) || Math.abs(players[enemyID].y - serverJson.idCordsY) > 30) {
            players[enemyID].x = parseInt(serverJson.idCordsX);
            players[enemyID].y = parseInt(serverJson.idCordsY);
            console.log('POS FIXED');
        }
    }

    console.log('dif: ' + Math.abs(players[enemyID].centerX - serverJson.idCordsX))
...
function endGame(serverJson) {
    if(serverJson.id == playerId) {
        game.state.start('gameOver', true, false, serverJson.player1Score, serverJson.player2Score);
        //game.state.start('gameOver', true, false, Math.round(game.world._height - playerSelf.y), Math.round(game.world._height - player.y));
    } else if (serverJson.id == enemyID) {
        game.state.start('gameOver', true, false, serverJson.player2Score, serverJson.player1Score);
    }
}
...
function spikeHit(sprite, tile) {
    console.log('CAKE AND GLORY FOR ANY AND ALL!')
    sprite.x = 50;
    sprite.y = 4750;
        //player.x = 50;
        //player.y = 4751;
...
mpGame.prototype = {
    ...
    preload: function () {
        /*
        // currently background is loaded from json instead
    ...
    create: function () {
        ...
        background = game.add.tileSprite(0, -200, 800, 600, 'bg9');
        background = game.add.tileSprite(0, -200, 800, 600, 'bg8');
        background = game.add.tileSprite(0, -200, 800, 600, 'bg7');
        background = game.add.tileSprite(0, -200, 800, 600, 'bg6');
        background = game.add.tileSprite(0, 0, 800, 600, 'bg5');
        background = game.add.tileSprite(0, 0, 800, 600, 'bg4');
        background = game.add.tileSprite(0, 0, 800, 600, 'bg3');
        background = game.add.tileSprite(0, 0, 800, 600, 'bg2');
        background = game.add.tileSprite(0, 0, 800, 600, 'bg1');
        ...
        map.setCollisionByExclusion([19, 13, 14, 15, 16, 46, 47, 48, 49, 50, 51]);
        layer = map.createLayer('Tile Layer 1');
        layer.resizeWorld();

        map.setTileIndexCallback(19, spikeHit, this);
        ...
        timerEvent = timer.add(Phaser.Timer.MINUTE * 5 + Phaser.Timer.SECOND * 15, this.endTimer, this);
        timer.start();
        this.timerLabel = game.add.text(100,100,'60', {font: "15px Arial", fill: "#19de65"});
    },
    ...
    update: function () {
            ...
            let jump = "false";
            let action = "none";
            let cordinatesPlayer = [players[playerId].x, players[playerId].y];
            let cordinatesEnemy = [players[enemyID].x, players[enemyID].y];
            ...
            if(startSignal){
                ...
                updatePlayerInput(playerName, playerId, action, jump, cordinatesPlayer, cordinatesEnemy);
            } else {
                playerStartSignal(playerName, playerId);
            }
    },
    render: function () {
        if (timer.running) {
            game.debug.text(this.formatTime(Math.round((timerEvent.delay - timer.ms) / 1000)), 2, 24, "#f72222");
            this.timerLabel.setText(Math.round((timerEvent.delay - timer.ms) / 1000));
    ...
    endTimer: function () {
        ...
        startSignal = false;
        gameStarted = false;
        notifyEnd(playerId, enemyID, Math.round((game.world._height - playerSelf.y)/10), Math.round((game.world._height - player.y)/10))
    },
...
let gameBootstrapper = {
    init: function (gameContainerElementId) {
        ...
        game.state.add('mpMakeAccount', regNewPlayer);
        game.state.add('userProfile', playerProfile);
